...
function isAuthorized(req: any): boolean {
  ...
  const candidate =
    String(req.headers?.["x-dashboard-token"] || "") ||
...
function applyProjectToDashboard(project: ProjectSettings) {
  dashboardState = {
    ...
    temperature: (project as any).temperature ?? dashboardState.temperature ?? 0.2,
    maxTokens: (project as any).maxTokens ?? dashboardState.maxTokens ?? 4096,
  };
}
...
async function checkDirectoryHealth(rawPath: string) {
  ...
  if (!isDirectory) return { resolvedPath, exists, isDirectory, readable, writable };
...
const server = createServer(async (req, res) => {
  ...
  if (pathname === "/fs/pick-directory" && req.method === "POST") {
    try {
      ...
      if (!picked) return json(res, { ok: false, canceled: true });
  ...
  if (pathname === "/fs/validate-directory" && req.method === "POST") {
    try {
      ...
      if (!raw) return json(res, { ok: false, error: "Missing path" }, 400);
  ...
  if (pathname === "/fs/recent-roots" && req.method === "GET") {
    try {
      ...
      const candidates = Array.from(
        new Set(
          [getWorkspaceRoot(), process.cwd(), ...projects.map((p: any) => p.rootDir || "")]
            .map((p) => String(p || "").trim())
            .filter(Boolean)
        )
      ).slice(0, 30);
      ...
      const checked = await Promise.all(
        candidates.map(async (root) => ({ root, ...(await checkDirectoryHealth(root)) }))
      );
  ...
  if (pathname === "/state/restore" && req.method === "POST") {
    try {
      ...
      if (!payload) throw new Error("Missing restore payload");
